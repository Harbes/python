...
# 按price分组(前n个交易日数据) ; y【low price在中国是不显著的，春节期间却是显著的，但是不稳定，对策略构建的日期较为敏感; 但是春节后显著】
t0=0
t1=9
num_by_=10
label_=[i+1 for i in range(num_by_)] #
...
# 下面这一小段代码有问题，tmp、mark_2、indi_2的shape不同
for l_ in label_:
...
clsprc.iloc[clsprc.index.get_loc(after_fes_data[-2][0])+t1]-opnprc.loc['2016-02-15']

mark_2.loc['2016-02-15'].mean()
